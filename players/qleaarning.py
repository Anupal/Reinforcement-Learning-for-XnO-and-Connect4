...
class TTTQLearningPlayer:
    def __init__(self, symbol, learning_rate=0.1, discount_factor=0.9):
        ...
        self.q_table = {}
        self.last_action = None  # Store the last action taken for debugging/analysis
    ...
    def update_q_table(self, state, action, next_state, done, reward):
        ...
        if state not in self.q_table:
            self.q_table[state] = np.random.rand(3, 3)  # Use a 3x3 matrix for actions
        if next_state not in self.q_table:
            self.q_table[next_state] = np.random.rand(3, 3)

        # Calculate the Temporal Difference (TD) error and update Q-values
        current_q_value = self.q_table[state][action]
        next_max_q_value = np.max(self.q_table[next_state]) if not done else 0  # No future reward if game is done
        qsa_observed = reward + self.discount_factor * next_max_q_value
    ...
    def choose_action(self, state, available_actions):
        ...
        # Extract Q-values for available actions and find the action with the highest Q-value
